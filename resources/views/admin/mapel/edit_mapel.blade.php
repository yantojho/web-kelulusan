...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.mapel.update', ['mapel' => $mapel->id ]) }}" method="post">
                    @csrf
                    @method('put')
                    <div class="mb-3">
                        ...
                        <select name="jenismapel_id" id="" class="form-control">
                            ...
                            <option value="{{ $item->id }}" {{ $item->id == $mapel->jenismapel_id ? 'selected' : '' }}>{{ $item->nm_jenis }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="nm_mapel" class="form-label">Nama Mapel</label>
                        <input type="text" class="form-control" value="{{ $mapel->nm_mapel }}" name="nm_mapel">
                    </div>
                    <div class="mb-3">
